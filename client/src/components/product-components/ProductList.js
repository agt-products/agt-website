...
function generateCategory(params) {
    return params.category3 ? params.category1 + "&" + params.category2 + "&" + params.category3 : params.category2 ? params.category1 + "&" + params.category2 : params.category1;
}

class ProductList extends Component {
    state = {
        category: generateCategory(this.props.match.params),
    ...
    componentWillMount() {
        this.count = null;
        this.props.getProductsByCategory({ category: this.state.category, limit: this.state.limit, page: this.state.page });
    ...
    componentWillUpdate(nextProps) {
        if(this.props.match.params !== nextProps.match.params) {
            this.setState({
                category: generateCategory(nextProps.match.params),
                page: 0
            });
        }
    ...
    componentDidUpdate(nextProps, nextState) {
        if(this.state.category !== nextState.category || this.state.limit !== nextState.limit || this.state.page !== nextState.page) {
            this.props.getProductsByCategory({ category: this.state.category, limit: this.state.limit, page: this.state.page });
            // this.props.getProductCategoryItemCount({ category: this.state.category });
        }
    }
            ...
            for(let i = 0; i < this.count; i++) {
                pages.push(
                    <PaginationItem key={`page${i + 1}`} active={parseInt(this.state.page) === i}>
                        <Link 
                            className="page-link" 
                            to={`/products/categories/${this.state.category.replace(/&/g, "/")}/${this.state.limit}/${i}`}
                        >
                            {i + 1}
                        </Link>
                    </PaginationItem>
        ...
        return (
            <Container fluid className="my-4">
                <Row>
                    ...
                    <Col lg="8">
                        <h2 className="mb-3">Point of Purchase Products</h2>
                        <Pagination size="sm">
                            <PaginationItem disabled={parseInt(this.state.page) === 0}>
                                <Link 
                                    className="page-link" 
                                    to={`/products/categories/${this.state.category.replace(/&/g, "/")}/${this.state.limit}/0`}
                                >
                                    First
                                </Link>
                            </PaginationItem>
                            <PaginationItem disabled={parseInt(this.state.page) < 1}>
                                <Link 
                                    className="page-link" 
                                    to={`/products/categories/${this.state.category.replace(/&/g, "/")}/${this.state.limit}/${parseInt(this.state.page) - 1}`}
                                >
                            ...
                            <PaginationItem disabled={parseInt(this.state.page) >= this.count - 1}>
                                <Link 
                                    className="page-link" 
                                    to={`/products/categories/${this.state.category.replace(/&/g, "/")}/${this.state.limit}/${this.count - 1}`}
                                >
                                    Last
                                </Link>
                            </PaginationItem>
                        </Pagination>
                        <ProductGallery products={this.props.categoryProducts.products} />
                    </Col>
                    <Col lg="2"></Col>
                </Row>
            </Container>
